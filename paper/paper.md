...
In particular, the gravitational wave observatories LIGO and Virgo have created their own models to use as templates in gravitational wave searches, with the main software for this being `LALSuite` [@lalsuite]. Various other research group have also created models, with some examples of recent sophisticated waveform models being `IMRPhenomXPHM` [@imrphenom] and `SEOBNRv4PHM` [@seobnr]. The waveform model of `Riroriro` that is based on @huerta2017 and @buskirk2019 is not as detailed or precise as these, but it is sufficiently accurate for use in evaluating the SNRs and detectability of populations of events. The code of `Riroriro` is also structured and commented in such a way that users could substitute in functions from other algorithms or even use the detectability modules on output waveforms from other sources, as long as these are formatted in the right way.
