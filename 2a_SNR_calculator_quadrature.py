...
ind_SNR = np.zeros((3))                 #holds three individual values
for x in range(3):                      #three detectors
    if x == 0:                          #Livingston
        noisearray=np.genfromtxt("May 2019 ASD curves/o3_l1.txt")
    elif x == 1:                        #Hanford
        noisearray=np.genfromtxt("May 2019 ASD curves/o3_h1.txt")
    elif x == 2:                        #Virgo
        noisearray=np.genfromtxt("May 2019 ASD curves/o3_v1.txt")
    ...
    def noiseratio(i):                                  #h/S part of SNR equation
        if noisearray[i,0] > freqmax or noisearray[i,0] < freqmin:
            ...
            #f to balance ASD units (from noise and from df)
    ...
    ind_SNR[x] = np.sqrt(4*sum(sumpart))

#quadrature sum of three SNR values    
quad_SNR = np.sqrt(ind_SNR[0]**2 + ind_SNR[1]**2 + ind_SNR[2]**2)
#average SNR of individual detector
mean_SNR = (ind_SNR[0] + ind_SNR[1] + ind_SNR[2])/3

print(quad_SNR,mean_SNR)
